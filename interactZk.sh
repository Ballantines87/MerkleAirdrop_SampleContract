# This bash script is needed because Foundry scripts right now don't work on zkSync

#!/bin/bash

# Define constants
DEFAULT_ZKSYNC_LOCAL_KEY="0x7726827caac94a7f9e1b160f7ea819f172f7b6f9d2a97f992c38edeab82d4110"
DEFAULT_ANVIL_KEY="0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80" # the one receiving the airdrop
...
# Compile and deploy BallToken contract
echo "Creating zkSync local node..."
npx zksync-cli dev start # to be able to run this command we need Docker installed and running
echo "Deploying Ball token contract..."
TOKEN_ADDRESS=$(forge create src/BallToken.sol:BallToken --rpc-url http://127.0.0.1:8011 --private-key ${DEFAULT_ZKSYNC_LOCAL_KEY} --legacy --zksync | awk '/Deployed to:/ {print $3}' )
...
CLEAN_SIGNATURE=$(echo "$SIGNATURE" | sed 's/^0x//') #we remove 0x at the start of the signature
echo -n "$CLEAN_SIGNATURE" >> signature.txt # we print that signature to a file called signature.txt

# Split signature script -> this is gonna split the signature into v, r, and s, taking the signature from the signature.txt file
